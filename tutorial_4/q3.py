...
for i in range(1,6):
    ...
    x_poly = poly_features.fit_transform(df1[['Shell weight']].to_numpy())
    
    lin_reg = LinearRegression()
    lin_reg.fit(x_poly, df1[['Rings']].to_numpy())
    
    x_new_poly = poly_features.transform(df2[['Shell weight']].to_numpy())
...
minimum = min(rmse_test_list)-0.01
maximum = max(rmse_test_list)+0.01
